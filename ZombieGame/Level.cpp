...
Level::Level(const std::string& fileName) {
	...
	if (file.fail()) {
		myGameEngine::fatalError("Failed to open" + fileName);
	}
	...
	myGameEngine::ColorRGBA8 whiteColor;
	whiteColor.r = 255;
	whiteColor.g = 255;
	whiteColor.b = 255;
	whiteColor.a = 255;
		...
		for (int x = 0; x < _levelData[y].size(); x++) {
			...
			case 'R':
				_spriteBatch.draw(destRect, uvRect, 
					myGameEngine::ResourceManager::getTexture("Textures/red_bricks.png").id,
					0.0f,  whiteColor);
				break;
			case 'G':
				_spriteBatch.draw(destRect, uvRect,
					myGameEngine::ResourceManager::getTexture("Textures/glass.png").id,
					0.0f, whiteColor);
				break;
			case 'L':
				_spriteBatch.draw(destRect, uvRect,
					myGameEngine::ResourceManager::getTexture("Textures/light_bricks.png").id,
					0.0f, whiteColor);
				break;
			case '@':
